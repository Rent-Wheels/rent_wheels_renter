import 'package:dartz/dartz.dart';
import 'package:rent_wheels_renter/core/models/reservation/reservation_model.dart';
import 'package:rent_wheels_renter/core/usecase/usecase.dart';
import 'package:rent_wheels_renter/src/cars/domain/repository/cars_repository.dart';

class GetCarRentalHistory extends UseCase<List<Reservation>, String> {
  final CarsRepository repository;

  GetCarRentalHistory({required this.repository});

  @override
  Future<Either<String, List<Reservation>>> call(String params) async {
    return await repository.getCarRentalHistory(carId: params);
  }
}
